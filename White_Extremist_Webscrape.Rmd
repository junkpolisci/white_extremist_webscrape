...
```{r Leftover Unigram LDA, eval=F, include=F}
u_lda3_topics <- tidy(unigram_lda3,
                      matrix = "beta")

u_lda4_topics <- tidy(unigram_lda4,
                      matrix = "beta")
```
...
```{r Unigram LDA 5 B, cache=T, echo=F, warning=F, message=F, fig.width=30, fig.height=60}
u_lda5_comparison %>% 
  group_by(pair_id) %>% 
...
```{r Unigram LDA 10, cache=T, echo=F, warning=F, message=F}
u_lda10_topics <- tidy(unigram_lda10,
                      matrix = "beta")
```
